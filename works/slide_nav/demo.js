...
function lwStartMove2(obj, oTarget, iType, fnCallBack, fnDuring)
{
    var fnMove=null;
    if(obj.timer)
    {
    ...
    switch(iType)
    {
        case LW_MOVE_TYPE.BUFFER:
            fnMove=lwDoMoveBuffer;
            break;
        case LW_MOVE_TYPE.FLEX:
            fnMove=lwDoMoveFlex;
            break;
    }
...
function lwDoMoveBuffer(obj, oTarget, fnCallBack, fnDuring)
{
    var bStop=true;
    var attr='';
    var speed=0;
...
function lwDoMoveFlex(obj, oTarget, fnCallBack, fnDuring)
{
    var bStop=true;
    var attr='';
    var speed=0;
...
window.onload=function ()
{
    ...
    for(i=0;i<aLi.length;i++)
    {
        aLi[i].onmouseover=function ()
        {
            ...
            lwStartMove2(oImg, {width: 153}, LW_MOVE_TYPE.FLEX, function (){
                css(this, 'marginLeft', -78);
            }, duringA);
            lwStartMove2(oA, {top: 56, paddingTop: 36, paddingBottom: 36}, LW_MOVE_TYPE.FLEX);
            lwStartMove2(oBg, {height: 115}, LW_MOVE_TYPE.BUFFER);
        };
        aLi[i].onmouseout=function ()
        {
            ...
            var oA=this.getElementsByTagName('a')[0];
            var oBg=this.getElementsByTagName('span')[1];
            lwStartMove2(oImg, {width: 0}, LW_MOVE_TYPE.BUFFER, function (){
                css(this, 'marginLeft', 0);
            }, duringA);
            lwStartMove2(oA, {top: 4, paddingTop: 8, paddingBottom: 0}, LW_MOVE_TYPE.BUFFER);
            lwStartMove2(oBg, {height: 0}, LW_MOVE_TYPE.BUFFER);
        };
    }
};